...
var RecentEvaluations = make([]dao.ServerEvaluation, 0)
...
func MakeEvaluationInDomainTW(waitTime time.Duration, domainName string, currentHour time.Time,
  evaluator func(time.Time, string) (dao.ServerEvaluation, error), db *sql.DB) (se dao.ServerEvaluation, changed bool, appErr APIError) {
	...
	if pendingEvaluation.Id != 0 {
      ...
      appErr = APIErrors.E601(err)
        ...
        appErr = APIErrors.E602(err)
			...
			// 1.1.2) In SSLabs, Is the server Evaluation in process?
...
func MakeEvaluationInDomain(domainName string, currentHour time.Time, evaluator func(time.Time, string) (dao.ServerEvaluation, error),
	db *sql.DB) (se dao.ServerEvaluation, changed bool, appErr APIError) {
    return MakeEvaluationInDomainTW(DomainEvaluationTW, domainName, currentHour, evaluator, db)
}
  ...
  if changed {
    ...
    if !se.EvaluationInProgress && !se.IsDown {
      for i := range sec.Servers {
        ...
        err = sec.Servers[i].UpdateInDB(db)
        if err != nil {
    			appErrs = append(appErrs, APIErrors.E601(err))
    			return
    		}
...
func ListRecentEvaluations(currentHour time.Time, db *sql.DB) (apiErrs []APIError) {
  ...
  if len(RecentEvaluations) == 0 {
    var err error
    RecentEvaluations, err = dao.ListRecentServerEvaluations(dao.DBConf)
